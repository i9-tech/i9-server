...
public interface PratoRepository extends JpaRepository<Prato, Integer> {
    ...
    @Query("select prato from Prato prato join prato.funcionario f where f.empresa = (select f2.empresa from Funcionario f2 where f2.id = :idFuncionario) and (:termoBusca is null or lower(prato.nome) like lower(concat('%', :termoBusca, '%'))) and (:disponivel is null or prato.disponivel = :disponivel) and (:setorId is null or prato.setor.id = :setorId) and (:categoriaId is null or prato.categoria.id = :categoriaId)")
    Page<Prato> buscarPratosComFiltros(@Param("idFuncionario") Integer idFuncionario, @Param("termoBusca") String termoBusca, @Param("disponivel") Boolean disponivel, @Param("setorId") Integer setorId, @Param("categoriaId") Integer categoriaId, Pageable pageable);
